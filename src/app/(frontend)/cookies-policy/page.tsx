...
import { getCookiesPolicyData } from "@/lib/payload";

export const metadata: Metadata = {
  title: "Cookies Policy | BellMedEx Healthcare Platform",
  description:
    "Understand how BellMedEx uses cookies and web technologies to ensure portal security, session integrity, and enhanced user experience.",
};

export default async function CookiesPolicyPage() {
  const data = await getCookiesPolicyData();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          <MotionWrapper variant="fadeIn">
            <div className="flex justify-center">
              <SectionBadge variant="blue" pulse icon={Cookie}>
                {hero.badge}
              </SectionBadge>
            </div>
          </MotionWrapper>
                  ...
                  <div key={sec.title || idx} className={idx > 0 ? "pt-8" : ""}>
                    ...
                    {sec.types && (
                      <div className="space-y-3 pt-2">
                        {sec.types.map((t) => (
                          <div key={t.name} className="p-3.5 rounded-xl bg-[#F8FAFC] border border-[#E2E6EC] space-y-1">
                            <p className="text-xs sm:text-sm font-bold text-[#0F172A] flex items-center gap-2">
                              <CheckCircle2 className="w-4 h-4 text-[#1D4ED8]" />
                              {t.name}
                            </p>
                            <p className="text-xs text-[#475569] pl-6">{t.desc}</p>
                          </div>
                        ))}
                      </div>
                    )}
