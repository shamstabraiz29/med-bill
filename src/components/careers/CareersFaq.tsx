...
import type { CareersFaqData } from "@/payload/types/careers";

interface CareersFaqProps {
  data?: CareersFaqData;
}

export default function CareersFaq({ data }: CareersFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const content = data || defaultCareersData.faq;
  const faqsList = content?.faqs && content.faqs.length > 0 ? content.faqs : defaultCareersData.faq?.faqs || [];
    ...
    <section className="relative w-full py-16 sm:py-20 bg-transparent overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-12 sm:mb-16">
          <SectionHeader
            badge={content?.badge || "CAREERS FAQ"}
            badgeVariant="indigo"
            align="center"
            title={
              <>
                {content?.titlePlain || "Frequently Asked"}{" "}
                <span className="text-[#1D4ED8]">{content?.titleHighlight || "Career Questions"}</span>
              </>
            }
            description={content?.description || "Find answers about working at BellMedEx, our hiring process, and employee benefits."}
            className="space-y-4"
          />
        </div>
        ...
        <div className="w-full">
          ...
          >
            {faqsList.map((faq, idx) => {
              ...
              const controlsId = `careers-faq-content-${idx}`;
              const headerId = `careers-faq-header-${idx}`;
