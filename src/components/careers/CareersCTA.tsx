...
export default function CareersCTA({ data }: CareersCTAProps) {
  ...
  const fullTitle = content?.title || "Ready to Shape the Future of Healthcare RCM?";
  let titlePlain = fullTitle;
  let titleHighlight = "";

  if (fullTitle.includes("Healthcare RCM?")) {
    const parts = fullTitle.split("Healthcare RCM?");
    titlePlain = parts[0];
    titleHighlight = "Healthcare RCM?";
  }
