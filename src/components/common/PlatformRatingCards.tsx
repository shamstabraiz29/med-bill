...
export interface PlatformRatingInput {
  platform: string;
  value: string;
  color?: string;
  subLabel?: string;
...
const SerchenIcon = (
  <svg
    className="h-4 w-4 shrink-0 text-[#1D4ED8]"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2.2"
...
export const DEFAULT_PLATFORM_RATINGS: PlatformRating[] = [
  {
    platform: "Google",
...
function getPlatformMeta(platform: string): Partial<PlatformRating> {
  ...
  switch (key) {
    ...
    case "BBB":
      return {
        platform: "BBB",
        subtitle: "A+ Rated Business",
        badge: "Accredited",
        icon: <Award className="h-4 w-4 shrink-0 text-[#1D4ED8]" aria-hidden="true" />,
        showStars: false,
      };
    case "ONC":
      return {
        platform: "ONC",
        subtitle: "Authorized Certification Body",
        badge: "Certified",
    ...
    default:
      return {
        ...
        subtitle: "",
        badge: "Verified",
        icon: <CheckCircle2 className="h-4 w-4 shrink-0 text-[#1D4ED8]" aria-hidden="true" />,
        showStars: false,
      };
  }
}

export function normalizePlatformRating(input: PlatformRatingInput): PlatformRating {
  const meta = getPlatformMeta(input.platform);

  return {
    platform: meta.platform || input.platform,
    subtitle: input.subLabel || input.label || meta.subtitle || "",
    value: input.value,
    maxScore: meta.showStars === false ? undefined : meta.maxScore || "/ 5.0",
    starColor: input.color || meta.starColor,
    badge: meta.badge || "Verified",
    icon: meta.icon,
    showStars: meta.showStars ?? Boolean(meta.starColor),
  };
}

export function normalizePlatformRatings(inputs?: PlatformRatingInput[]): PlatformRating[] {
  if (!inputs?.length) {
    return DEFAULT_PLATFORM_RATINGS;
  }

  return inputs.map(normalizePlatformRating);
}
...
export function PlatformRatingCard({ rating }: { rating: PlatformRating }) {
  const showStars = rating.showStars !== false && Boolean(rating.starColor);
  ...
  return (
    <article className={cardClassName}>
      ...
      {rating.subtitle ? (
        <div className="text-[11px] font-medium text-[#64748B]">{rating.subtitle}</div>
      ) : null}
    </article>
  );
}

interface PlatformRatingCardsProps {
  ratings?: PlatformRatingInput[];
  className?: string;
...
export default function PlatformRatingCards({
  ratings,
  className,
  gridClassName,
  staggerDelay = 0.08,
  ariaLabel = "Platform ratings",
}: PlatformRatingCardsProps) {
  const resolvedRatings = normalizePlatformRatings(ratings);
  ...
  return (
    <MotionWrapper
      ...
      className={cn(
        "grid w-full grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:gap-5",
        resolvedRatings.length === 3 && "sm:grid-cols-3",
        resolvedRatings.length === 4 && "md:grid-cols-2 xl:grid-cols-4",
        gridClassName,
        className
      )}
